...
SENSORS = {
    ...
    "qcmos": {
        "bb": "wcc_hwk_bb_throughput.csv",  # "Lazuli_WCC_kepler_20251010_eol_qCMOS",
...
sensor_info = {
    ...
    "10": {
        "center": (-886.5, 12.0),
        "sensorfilter": "zwo:halpha",
        "sensor": "imx455",
        "focus_level": "0wave",
        "implemented": False,
        "filter_label": "H-alpha",
    },
    "11": {
        "center": (-533.5, 12.0),
        "sensorfilter": "zwo:hbeta",
        "sensor": "imx455",
        "focus_level": "0wave",
        "implemented": False,
        "filter_label": "H-Beta",
...
_SENSORFILTER_FOCUS: dict = {
    entry["sensorfilter"]: entry["focus_level"] for entry in sensor_info.values()
}

_SENSORFILTER_IMPLEMENTED: dict = {
    entry["sensorfilter"]: entry["implemented"] for entry in sensor_info.values()
}
...
def get_pickles_spectrum_filename(spectral_type, fullpath=True):
    ...
    """
    filename = (
        PICKLES_MAPPING[PICKLES_MAPPING["spt"].values == spectral_type][
            "filename"
        ].values[0]
        + ".fits"
    )
    ...
    if not filename:
        raise ValueError(
            f"No spectrum found for {spectral_type=}. Available SPT are {PICKLES_MAPPING['spt'].values}."
        )
...
def read_config(filename, source="config"):
    ...
    if extension is None or len(extension) == 0:
        raise ValueError(
            f"no extension associated to given filename {filename=}. It cannot be loaded"
        )
    ...
    if band not in kind_sensors:
        ...
        if throughput_filter is None:
            raise NotImplementedError(
                "{kind_sensors} {band} sensor exists but no throghputcurve implemented yet."
            )
...
def expand_path(filename, source=None, test_extension=False):
    ...
    if os.path.isfile(filename):  # filename includes a path
        ...
        if extension is None or len(extension) == 0:
            fname = f"{fname}.toml"
        elif test_extension and (
            extension not in [".toml", ".csv", ".parquet"]
        ):  # specify here list of accepted extensions.
            raise NotImplementedError(f"Unknown configuration extension {extension=}.")

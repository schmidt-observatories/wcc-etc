...
class TestPsfAwareSaturation:
    def test_defocus_peak_fraction_far_below_airy(self):
        sim_def = wcc_etc.Simulation.from_sensorfilter("zwo:bb2", make_scene(name="G2V", bandpass="johnson_v", mag=15.0))
    ...
    def test_is_saturated_agrees_with_image_snr_flag(self):
        sim = wcc_etc.Simulation.from_sensorfilter("zwo:bb2", make_scene(name="G2V", bandpass="johnson_v", mag=13.0))
    ...
    def test_peak_pixel_array_shape(self):
        sim = wcc_etc.Simulation.from_sensorfilter("zwo:r", make_scene(name="G2V", bandpass="johnson_v", mag=16.0))
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            p2 = (

...
class TestSimulationInit:
    def test_fullkey_to_element_and_key_qualified(self):
        element, key = Simulation._fullkey_to_element_and_key("sensor__gain")
        assert element == "sensor"
        assert key == "gain"

    def test_fullkey_to_element_and_key_simple(self):
        element, key = Simulation._fullkey_to_element_and_key("time")
        assert element is None
        assert key == "time"

    def test_get_parameter_and_mutable_parameters(self):
        sim = Simulation(
...
class TestPeakPixel:
    ...
    def test_electrons_excludes_bias(self, sim):
    ...
    def test_accepts_array_time(self, sim):
...
class TestGetImageSnr:
    ...
    def test_scalar_returns_floats(self, sim):
        out = sim.get_image_snr(time=60)
    ...
    def test_no_background_path(self):
    ...
    def test_array_time_monotonic_and_matches_scalar(self, sim):
    ...
    def test_array_time_optimize(self, sim):
    ...
    def test_delegates_from_get_snr(self, sim):
    ...
    def test_snr_airy_deprecated_matches_analytic(self, sim):
        with pytest.warns(DeprecationWarning):
...
class TestRenderCache:
    ...
    def test_render_bundle_cached(self, sim, monkeypatch):
    ...
    def test_update_invalidates_cache(self, sim):
    ...
    def test_update_jitter_clears_caches(self, sim):
    ...
    def test_set_scene_clears_cache(self, sim):
...
class TestMagsSweep:
    ...
    def test_scalar_matches_rebuild(self, sim):
    ...
    def test_array_matches_rebuild(self, sim):
    ...
    def test_array_length_one(self, sim):

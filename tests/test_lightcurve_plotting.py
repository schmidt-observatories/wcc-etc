...
def test_bokeh_both_off_does_not_raise():
    out = plot_lightcurve_bokeh(_lc(), show_noise=False, show_model=False,
                                return_="components")
    assert isinstance(out, tuple) and len(out) == 2
...
def test_bokeh_raw_array_flux_err_does_not_raise():
    ...
    out = plot_lightcurve_bokeh(time=lc.time, flux=lc.flux,
                                flux_clean=lc.flux_clean, flux_err=err,
                                return_="components")
    assert isinstance(out, tuple) and len(out) == 2

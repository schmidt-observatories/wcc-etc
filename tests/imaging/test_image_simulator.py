...
class TestImageSimulator:
    def test_clean_flux_shape(self):
        imsim = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=15), npix=128
        )
    ...
    def test_same_seed_gives_same_noise(self):
        imsim = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=15), npix=64
        )
    ...
    def test_faint_star_not_saturated(self):
        faint = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=20), npix=64
        )
        assert not faint.simulate(time=1, add_noise=False).saturation_mask.any()

    def test_bright_star_is_saturated(self):
        bright = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=6), npix=64
        )
        assert bright.simulate(time=100, add_noise=False).saturation_mask.any()

    def test_sensor_pixel_scales_differ(self):
        sony = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=15), npix=64
        )
        hwk = ImageSimulator.from_sensor_and_scene(
            "qcmos:r", make_scene(mag=15), npix=64
        )
    ...
    def test_defocus_psf_output_shape(self):
        imsim = ImageSimulator.from_sensor_and_scene(
            "sony:r", make_scene(mag=15), npix=300
        )
        res = imsim.simulate(

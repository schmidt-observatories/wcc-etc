...
class TestPeakPixelFraction:
    def test_peak_pixel_fraction_in_unit_interval(self):
        sim = wcc_etc.Simulation.from_sensor_and_scene(
            "sony:r", make_scene(name="G2V", bandpass="johnson_v")
        )
    ...
    def test_defocus_peak_fraction_below_in_focus(self):
        sim_def = wcc_etc.Simulation.from_sensorfilter(
            "zwo:bb2", make_scene(name="G2V", bandpass="johnson_v")
        )
        sim_foc = wcc_etc.Simulation.from_sensorfilter(
            "zwo:r", make_scene(name="G2V", bandpass="johnson_v")
        )

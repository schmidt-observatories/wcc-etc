...
class TestSensor:
    ...
    def test_property_units_and_values(self):
        s = self._make()
    ...
    def test_bit_depth_and_adc_max(self):
        s = self._make(bit_depth=16)
        assert s.bit_depth == 16
        assert s.adc_max == 65535 * u.ct

    def test_bias_level_defaults_to_zero(self):
    ...
    def test_bit_depth_and_bias_level_are_updatable(self):
        s = self._make(bit_depth=16)
        s.update(bit_depth=12, bias_level=50)
        assert s.adc_max == 4095 * u.ct
        assert s.bias_level == 50 * u.ct

    def test_from_config_reads_bit_depth_and_bias_level(self):
        cfg = {
            "throughput": None,
            "pixel_size": 4,
            "sensor_area": 50,

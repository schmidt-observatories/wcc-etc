...
def test_bundle_rates_match_components():
    ...
    assert b["source_rate_total"] == __import__("pytest").approx(
        comp["source_rate_total"], rel=1e-6
    )
    assert b["background_rate_per_pix"] == __import__("pytest").approx(
        comp["background_rate_per_pix"], rel=1e-6
    )
    assert b["diffuse_rate_per_pix"] == __import__("pytest").approx(
